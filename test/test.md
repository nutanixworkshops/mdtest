...
*Basically - why do people love Nutanix, and what can we do to make
their experience better.*

From these studies, we were able to identify much of what a typical "Day
...
that have brought Nutanix success:

-   The industry's best HCI software - AOS
-   An unmatched consumer-grade management experience in Prism

Our Private Cloud story is built on the strongest *foundation* (sorry,
couldn't help it) possible.

**In this lab you will follow a day in the life of Carol O'Kay, a 10
...
support the company's retail storefronts.**

## Configuring Storage

In this brief exercise, you will experience how IT generalists can
...
managing traditional SAN storage.

1.  Using your **Cluster Assignment Spreadsheet**, identify your **Prism Element** cluster IP.
...
1.  In a browser, open **Prism Element** and log in using the following
    local user credentials:

    -   **Username** - admin
    -   **Password** - techX2020!

    [![](images/1.png)](https://www.google.com)

1.  Select **Storage** from the drop down menu.

    ![](images/2.png)

1.  Mouse over the **Storage Summary** and **Capacity Optimization**
    widgets for explanations of what each is displaying.

    It's important to understand that Nutanix considers the **Data
    Reduction** ratio as savings ONLY from compression, deduplication,
    and erasure coding. The **Overall Efficiency**, comparable to what
...
1.  Select **Table** and click **+ Storage Container**.

    Storage Containers represent logical policies for storage, allowing
...
1.  Provide a unique **Name** for the **Storage Container**, and click
    **Advanced Settings** to explore additional configuration options.

    ![](images/5.png)

    Nutanix provides different ways to optimize storage capacity that
    ...
    container level, so different containers can use different settings.

    **Compression**

    > Nutanix provides both inline and post-process data compression.
    ...
    > the Extent Store, after compression delay is met.
    >
    > Compression provides on-disk space savings for applications such
    ...
    **Erasure Coding**
    ...
    > To provide a balance between availability and the amount of
    ...
    >
    > Turn on EC-X for non-mission-critical workloads and workloads that
    ...
    > more information refer to [application specific best practice
    > guides](https://portal.nutanix.com/page/documents/solutions/list/).

    **Deduplication**

    > When enabled, DSF does capacity-tier and performance-tier
    ...
    > with the same fingerprint are satisfied directly from cache.
    >
    > Deduplication is recommended for full clones, P2V migrations and
    > Persistent Desktops.

    **Redundancy Factor**

    > Redundancy Factor controls the number of data copies. Observe that
    ...
    > **note**
    >
    > For more information on how Nutanix protects your data or
    > implements data reduction, click the diagram below to review the
    > relevant section of the Nutanix Bible.
    >
    > ![](https://nutanixbible.com/imagesv2/data_protection.png)

1.  Click **Save** to create the storage and mount it to all available
    hosts within the cluster.

    In vSphere or Hyper-V environments, creating the Storage Container
    will also automate the process of mounting the storage to the
    hypervisor.

1.  Select an existing Storage Container, such as
    **buckets-ctr-data...**, and review the individual savings from
    different data reduction/avoidance features, as well as the
    **Effective Capacity**, which is a projection of available storage
    based on the overall efficiency. These values are found in the
    **Storage Container Details** table.

    Unfortunately it is not possible to easily test data resiliency
    ...
    node in the cluster is unexpectedly lost.
...
In this exercise Carol will use Prism to configure a new VM Network for
the cluster.

AHV leverages Open vSwitch (OVS) for all VM networking. OVS is an open
...
> **note**
>
> Typically, Hosted POC clusters provide 2x /25 VLANs. In order to
...
1.  Select **VM** from the **Prism Element** drop down menu.
2.  Select **Network Config**.

    ![](images/9.png)

3.  Click **+ Create Network** and fill out the following fields, using
    the **User** specific network details in clusterassignments:

    -   **Name** - *Refer to* clusterassignments
    -   **VLAN ID** - *Refer to* clusterassignments
    -   Select **Enable IP Address Management**
    -   **Network IP Address / Prefix Length** - *Refer to*
        clusterassignments
    -   **Gateway IP Address** - *Refer to* clusterassignments
    -   **Domain Name Servers** - *Refer to* clusterassignments
    -   **Domain Search** - ntnxlab.local
    -   **Domain Name** - ntnxlab
    -   Select **+ Create Pool**
    -   **Start Address** - *Refer to* clusterassignments
    -   **End Address** - *Refer to* clusterassignments
    -   Click **Submit**

    ![](images/8.png)

    Note that AHV is capable of providing integrated DHCP services
    ...
    reservations when adding virtual NICs to VMs.

4.  Click **Save**.

    The configured virtual network will now be available across all
...
5.  Close the **Network Configuration** window.

    You're done - simple stuff! You will continue to use this network
    for the following exercises.

Responding to VM Creation Requests
----------------------------------

Virtualization administrators are commonly tasked with deployment of new
VMs. In this exercise, Carol walks through deployment of an AHV VM in
Prism as a Nutanix administrator.

1.  Return to the **VM** page in **Prism Element** from the drop down
    menu.
2.  Click **+ Create VM**.

    ![](images/10.png)

3.  Fill out the following fields to complete the user VM request:
    ...
    Similar to public cloud providers, Nutanix AHV provides an Image
    ...
    Note that the VM creation wizard also provides the ability to
    specify a Unattend.xml file for Windows Sysprep automation, or
    Cloud-Init file for Linux OS configuration.

4.  Click **Save** to create the VM.

    > **note**
    >
    > Many VM operations, including VM creation can be scripted using
    > the AHV CLI, `acli`. Certain features, such as Secure Boot and
    > vNUMA can currently only be enabled for a VM through the command
    > line. The ACLI Reference Guide can be found
    > [here](https://portal.nutanix.com/#/page/docs/details?targetId=Command-Ref-AOS-v5_16:acl-acli-vm-auto-r.html).
    >
    > You can SSH into any of your Nutanix CVMs and attempt creating an
    > additional VM using `acli`.

5.  Using the search field at the top of the table, filter for the
    requested VM. Select the VM and click **Power On** from the list of
    actions below the table.

    ![](images/12.png)

6.  Once the VM has completed booting, note the **IP Address**.

    ![](images/11.png)

    With previous infrastructure, Carol has had issues with newly
    ...
    machine she has provisioned.

7.  Try it yourself by selecting the **Network** page from the **Prism
    Element** drop down menu and filtering by VLAN or VM name.
...
While Prism and `acli` provide simple workflows for creating VMs, Carol
...
infrastructure, and attending her son's soccer games.

In the following exercises, Carol is going to up her Private Cloud game
and bring IaaS self-service to her users leveraging native capabilities
in **Prism Central**.

1.  Return to the **Home** page of **Prism Element**.
2.  Access **Prism Central** by clicking the **Launch** button and
...
A **Category** is a key value pair. Categories are assigned to entities
...
entities that are assigned a specific category value.

For example, you might have a Department category that includes values
...
Central allows you to quickly view any established relationships.

In this exercise you'll create a custom category for Carol to help align
access to the proper resources for the Fiesta app team.

1.  In **Prism Central**, select bars **\> Virtual Infrastructure \>
    Categories**.

    ![](images/14.png)

2.  Click **New Category** and fill out the following fields:
    -   **Name** - *Initials*-Team
    -   **Purpose** - Allowing resource access based on Application Team
    -   **Values** - Fiesta

3.  Click **Save**.
4.  Select the existing **Environment** category and note the available
    values. **Environment** is a **SYSTEM** category, and while you can
    add additional values, you cannot modify or delete the Category or
    any of its out of the box values.

    ![](images/16.png)

5.  Select bars **\> Virtual Infrastructure \> VMs**.
6.  Using the checkboxes, select the **AutoAD**, **DDC**, and
    **NTNX-BootcampFS-1** VMs and click **Actions \> Manage
    Categories**.

    ![](images/17.png)

7.  In the search bar, begin typing **Environment** and select the
    **Production** value.
    ...
    > **note**
    >
    > For categories tied to Security, Protection, or Recovery policies,
    > related policies will appear in this window to show the impact of
    > applying a Category to an entity.

8.  Click **Save**.
9.  Select the *Initials***-WinToolsVM** provisioned by Carol in the
    previous exercise, and click **Actions \> Manage Categories**.
    Assign the *Initials***-Team: Fiesta** category and **Save**.

### Exploring Roles

By default, Prism Central ships with several Roles that map to common
user personas. Roles define what actions a user can perform, and are
mapped to categories or other entities.

Carol needs to support two types of users working on the Fiesta team,
...
who have very limited capabilities to modify each environment.

1.  In **Prism Central**, select bars **\> Administration \> Roles**.

    The built-in Developer role allows users to create and modify VMs,
    create, provision, and manage Calm Blueprints, and more.

2.  Select the built-in **Developer** role and optionally review the
    approved actions for the role. Click **Manage Assignment**.

    ![](images/19.png)

3.  Under **Users and Groups**, specify the **SSP Developers** User
    Group which should be automatically discovered from the
    NTNXLAB.local domain.

    > **note**
    >
    > This may already have been completed by another user, but you'll
    > need to ensure the proper entities are also added below.

4.  Under **Entities**, use the drop down menu to specify the following
    resources:

    -   **AHV Cluster** - *Your Assigned Cluster*
    -   **AHV Subnet** - *Your Assigned User VLAN*
    -   **Category** - Environment:Testing, Environment:Staging,
        Environment:Dev, *Initials*-Team:Fiesta

    ![](images/20.png)

5.  Click **Save**.

    The default Operator roll includes the ability to delete VMs and
...
6.  Select the built-in **Operator** role and click **Duplicate**.
7.  Fill out the following fields and click **Save** to create your
    custom role:
...
8.  Refresh **Prism** and select your **SmoothOperator** role. Click
    **Manage Assignment**.
9.  Create the following assignment:

    -   **Users and Groups** - operator01
    -   **Entity Categories** - Environment:Production,
        Environment:Testing, Environment:Staging, Environment:Dev

    Operator01 is a user who has access to all VMs tagged with any of
    the Environment categories, but lacks generic access to specific
    clusters.

10. Click **New User** to add an additional assignment to the same role:

    -   **Users and Groups** - operator02
    -   **Entity Categories** - EnvironmentDev, *Initials*-Team:Fiesta

    Operator02 is a user who sees all VMs tagged with either the Dev or
    Fiesta category values.

    ![](images/22.png)

11. Click **Save**.
12. For infrastructure administrators such as Carol, you can map AD
    users to the **Prism Admin** or **Super Admin** roles through
    selecting bars **\> Prism Central Settings \> Role Mapping** and
    adding a new **Cluster Admin** or **User Admin** mapping to AD
    accounts.
...
The previous exercises are sufficient to provide basic VM creation
...
Nutanix Calm allows you to build, provision, and manage your
applications across both private (AHV, ESXi) and public cloud (AWS,
Azure, GCP) infrastructure.

In order for non-infrastructure administrators to access Calm, allowing
them to create or manage applications, users or groups must first be
assigned to a **Project**, which acts as a logical container to define
...
1.  In **Prism Central**, select bars **\> Services \> Calm**.
2.  Select **Projects** from the lefthand menu and click **+ Create
    Project**.
...
3.  Fill out the following fields:
    ...
    > **note**
    >
    > Adding the User/Group mappings before adding the Infrastructure
    > can cause adding the Infrastructure to fail. To avoid this, add
    > the Infrastructure before the User/Group mappings.

    -   **Project Name** - *Initials*-FiestaProject
    -   Under **Infrastructure**, select **Select Provider \> Nutanix**
    -   Click **Select Clusters & Subnets**
    -   Select *Your Assigned Cluster*
    -   Under **Subnets**, select **Primary**, **Secondary**, and *Your
        Assigned User VLAN*, and click **Confirm**
    -   Mark *Your Assigned User VLAN* as the default network by
        clicking the star
...
4.  Click **Save & Configure Environment**.

`This will redirect you to the Envrionments page, but there is nothing needed to configure here. Move onto the next step.`

Note that only **Operator02** was given access to the **Calm** project,
rather than all Operator accounts.
...
You can use Blueprints to model applications of various complexities;
from simply provisioning a single virtual machine to provisioning and
managing a multi-node, multi-tier application.

While developer users will have the ability to create and publish their
own Blueprints, Carol wants to provide a common Fiesta Blueprint used by
the team.

1.  [Download the Fiesta-Multi Blueprint by right-clicking
    here](https://raw.githubusercontent.com/nutanixworkshops/ts2020/master/pc/dayinlife/Fiesta-Multi.json).
2.  From **Prism Central \> Calm**, select **Blueprints** from the
    lefthand menu and click **Upload Blueprint**.

    ![](images/25.png)

3.  Select **Fiesta-Multi.json**.
4.  Update the **Blueprint Name** to include your initials. Even across
    different projects, Calm Blueprint names must be unique.
5.  Select your Calm project and click **Upload**.

    ![](images/26.png)

6.  In order to launch the Blueprint you must first assign a network to
    the VM. Select the **NodeReact** Service, and in the **VM**
    Configuration menu on the right, select *Your Assigned User VLAN* as
    the **NIC 1** network.
7.  Specify the *Initials***-Team: Fiesta** and **Environment: Dev**
    categories for the **NodeReact** Service.

    ![](images/27.png)

8.  Repeat the **NIC 1** and **Category** assignment for the **MySQL**
    Service.
9.  Click **Credentials** to define a private key used to authenticate
    to the CentOS VM that will be provisioned by the Blueprint.

    ![](images/27b.png)

10. Expand the **CENTOS** credential and use your preferred SSH key, or
    paste in the following value as the **SSH Private Key**:
...
11. Click **Save** and click **Back** once the Blueprint has completed
    saving.

    Within minutes, Carol has laid the groundwork to provide virtual
...
Meet Dan. Dan is a member of the Fiesta Engineering team, is currently
...
Dan needs to work have each taken several days.

Dan has resorted to deploying VMs outside of the corporate network on
...
going to fault him for it.

Carol to the rescue - she encourages Dan to follow the exercise below to
allow him to easily deploy resources within the Fiesta project through
Prism.

1.  Log out of the local **admin** account and log back into **Prism
    Central** with Dan's credentials:
    ...
    > **note**
    >
    > If you experience a slow login, try logging in using an
    > Incognito/Private browsing session.

2.  Select the bars menu and note that you now have significantly
    restricted access to the environment.
3.  On the **VMs** page, you should already see your
    *Initials***-WinToolsVM** as available to be managed by Dan.
4.  Select the VM and note Dan can get basic metrics associated with his
    VM, as well as control the VM configuration, power operations, and
    even delete the VM.

    ![](images/29.png)

    There are two workflows that could be followed for self-service
    ...
    tools required as part of his development workflow.

5.  Click **Create VM** and fill out the following fields to provision a
    traditional virtual machine, similar to the manual VM deployment
    process Carol followed earlier in the lab:
    -   **Create VM from** - Disk Images
    -   **Select Disk Images** - Linux\_ToolsVM.qcow2
...
6.  Click **Save** and note the VM is immediately powered on following
    creation.

    In addition to the tools VM, Dan also desires to deploy
...
7.  Select bars **\> Services \> Calm**.
8.  Select **Blueprints** from the left hand menu and open the
    **Fiesta-Multi** Blueprint.

    ![](images/30.png)

    > **note**
    >
    > If you're unfamiliar with Calm Blueprints, take a moment to
    > explore the following key components of the **Fiesta-Multi**
    > Blueprint:
    >
    > -   Select either the **NodeReact** or **MySQL** service and
    >     review the **VM** configuration in the configuration pane on
    >     the right hand of the screen.
    >
    >     > ![](images/31.png)
    >
    > -   Select the **Package** tab and click **Configure Install** to
    >     view the installation tasks for the selected service. These
    >     are the scripts and actions associated with the configuration
    >     of each Service or VM.
    >
    >     > ![](images/32.png)
    >
    > -   Under **Application Profile**, select **AHV** and view the
    ...
    >     > ![](images/33.png)
    >
    > -   Select the **Create** Action under **Application Profile** to
    ...
    >     begin until the MySQL database is running.
    >
    >     > ![](images/34.png)
    >
    > -   Click **Credentials** in the toolbar at the top of the
    >     Blueprint Editor and expand the existing **CENTOS**
    >     credential. Blueprints can contain multiple credentials which
    >     can be used to authenticate to VMs to execute scripts, or
    >     securely pass credentials directly into scripts.
    >
    >     > ![](images/35.png)
    >
    > -   Click **Back**.

9.  Click **Launch** to provision an instance of the Blueprint.

    ![](images/36.png)

10. Fill out the following fields and click **Create**:

    -   **Name of of the Application** - *Initials* -FiestaMySQL
    -   **db\_password** - nutanix/4u

    ![](images/37.png)

11. Select the **Audit** tab to monitor the deployment of the Fiesta
    development environment. Complete provisioning of the app should
    take approximately

    ![](images/38.png)

12. While the application is provisioning, open bars **\> Administration
    \> Projects** and select your project.
13. Review the **Summary**, **Usage**, **VMs**, and **Users** tabs to
...
14. Return to **Calm \> Applications \>** *Initials***-FiestaMySQL** and
    wait for the application to move from **Provisioning** to
    **Running**. Select the **Services** tab and select the
    **NodeReact** Service to obtain the IP of the web tier.

    ![](images/40.png)

15. Open http://\<*NodeReact-VM-IP*\> in a new browser tab and validate
    the app is running.

    ![](images/41.png)

    Instead of filing tickets and waiting days, Dan was able to get his
    ...
    and eat vegetables with his dinner. Go, Dan!

Operator Workflows
------------------

Meet Ronald and Elise. Ronald works as a Level 3 engineering with the
...
1.  Log out of the **devuser01** account and log back into **Prism
    Central** with Ronald's credentials:
    -   **User Name** - <operator01@ntnxlab.local>
    -   **Password** - nutanix/4u

2.  As expected, all VMs with **Environment** category values assigned
    are available. Note that you have no ability to **Create** or
    **Delete** VMs, but the abilities to power manage and change VM
...
3.  Log out of the **operator01** account and log back into **Prism
    Central** with Elise's credentials:
    -   **User Name** - <operator02@ntnxlab.local>
    -   **Password** - nutanix/4u

4.  Note that only resources tagged with the *Initials***-Team: Fiesta**
    category are available to be managed.

    ![](images/43.png)

5.  Elise receives an alert that memory utilization is high on the
    **nodereact** VM. Update the configuration to increase memory and
    power cycle the VM.

Using Entity Browser, Search, and Analysis
------------------------------------------

Now that Carol has freed up time to focus on replacing additional legacy
...
entities across multiple clusters in a Nutanix environment.

1.  Log out of the **operator02** account and log back into **Prism
    Central** with Carol's AD credentials:
    -   **User Name** - <adminuser01@ntnxlab.local>
    -   **Password** - nutanix/4u

2.  Open bars **\> Infrastructure \> VMs**. Prism Central's **Entity
    Browser** provides a robust UI for sorting, searching, and viewing
    entities such as VMs, Images, Clusters, Hosts, Alerts, and more!
3.  Select **Filters** and explore the available options. Specify the
    following example filters:
    ...
    Take notice of other helpful filters available such as VM
    efficiency, memory usage, and storage latency.

4.  Select all of the filtered VMs and click the **Label** icon to apply
    a custom label to your group of filtered VMs (e.g. *Initials* AHV
    Fiesta VMs).

    ![](images/44.png)

5.  Clear all filters and select your new label to quickly return to
...
6.  Select the **Focus** dropdown to access different out of box views.
    Which view should be used to understand if your VMs are included as
    part of a DR plan?
7.  Click **Focus \> + Add Custom** to create a VM view (e.g.
    *XYZ-VM-View*) that displays **CPU Usage**, **CPU Ready Time**, **IO
    Latency**, **Working Set Size Read**, and **Working Set Size
    Write**. Such a view could be used to helping to spot VM performance
    problems.

    ![](images/46.png)

8.  To fully appreciate the power of Prism Central for searching,
    sorting, and analyzing entities, view the following brief video:
...
While not a daily activity, Carol previously dedicated as much as 40% of
...
applying her infrastructure software updates.

Unfortunately in a shared cluster environment, you're not able to test
LCM directly. To become more familiar with LCM's capabilities and ease
of use, click through each of the interactive demos available below.

### 5.11 Prism Element LCM Interactive Demo

![](https://demo-captures.s3-us-west-1.amazonaws.com/pe-5.11-lcm/story_content/thumbnail.jpg)

### 5.11 Prism Central LCM Interactive Demo

![](https://demo-captures.s3-us-west-1.amazonaws.com/pc-5.11-lcm/story_content/thumbnail.jpg)

Next Steps
----------

In under 2 hours, we've shown you how Prism delivers a frictionless
...
your Private Cloud through Nutanix Calm.

Private Clouds aren't built on IaaS, self-service, and application
...
operations capabilities through its additional **Prism Pro** features,
consolidate storage technologies with **Files**, native
microsegmentation with **Flow**, and more!
